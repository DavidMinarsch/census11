...
# http://geoportal.statistics.gov.uk/datasets?q=Ward%20to%20Westminster%20Parliamentary%20Constituency%
# 20to%20Local%20Authority%20District%20Lookup&sort=name
# http://geoportal.statistics.gov.uk/datasets/65544c20a5804677a2594fe750bf4482_0

# load data:
census_11_mlsoa_male_white = pd.read_csv(script_dir + '/input/mlsoa_male_white.csv')
census_11_mlsoa_male_asian = pd.read_csv(script_dir + '/input/mlsoa_male_asian.csv')
census_11_mlsoa_male_black = pd.read_csv(script_dir + '/input/mlsoa_male_black.csv')
census_11_mlsoa_male_mixed = pd.read_csv(script_dir + '/input/mlsoa_male_mixed.csv')
census_11_mlsoa_male_other = pd.read_csv(script_dir + '/input/mlsoa_male_other.csv')
census_11_mlsoa_female_white = pd.read_csv(script_dir + '/input/mlsoa_female_white.csv')
census_11_mlsoa_female_asian = pd.read_csv(script_dir + '/input/mlsoa_female_asian.csv')
census_11_mlsoa_female_black = pd.read_csv(script_dir + '/input/mlsoa_female_black.csv')
census_11_mlsoa_female_mixed = pd.read_csv(script_dir + '/input/mlsoa_female_mixed.csv')
census_11_mlsoa_female_other = pd.read_csv(script_dir + '/input/mlsoa_female_other.csv')
mlsoa11_to_w16_lookup = pd.read_csv(script_dir + '/input/Middle_Layer_Super_Output_Area_'
                                    + '2011_to_Ward_to_Local_Authority_District_December_2016'
                                    + '_Lookup_in_England_and_Wales.csv', encoding='latin-1')
w16_to_pc16_lookup = pd.read_csv(script_dir + '/input/Ward_to_Westminster_Parliamentary_Constituency'
                                 + '_to_Local_Authority_District_December_2016_Lookup_in_the_United_Kingdom.csv')
...
for sex in ['male', 'female']:
    for ethnicity in ['white', 'asian', 'black', 'mixed', 'other']:
        ...
        census_11_mlsoa.rename(columns={'mnemonic': 'MSOA11CD', '2011 super output area - middle layer': 'MSOA11NM'},
                               inplace=True)
        ...
        census_11_mlsoa11_w16_pc16.to_csv(script_dir + '/output/mlsoa11_w16_pc16_' + sex + '_' + ethnicity + '.csv')
        # For now merge low ages:
        census_11_mlsoa11_w16_pc16['Age 15'] = census_11_mlsoa11_w16_pc16['Age 0 to 4']\
            + census_11_mlsoa11_w16_pc16['Age 5 to 7']\
            + census_11_mlsoa11_w16_pc16['Age 8 to 9']\
            + census_11_mlsoa11_w16_pc16['Age 10 to 14']\
            + census_11_mlsoa11_w16_pc16['Age 15']
        census_11_mlsoa11_w16_pc16.drop(['Age 0 to 4', 'Age 5 to 7', 'Age 8 to 9', 'Age 10 to 14'],
                                        axis=1,
                                        inplace=True)
    ...
    for e in range(0, n_ethnicity):
        for s in range(0, n_sex):
            for a in range(0, n_age):
                ...
                data = pd.DataFrame({'sex': [s], 'ethnicity': [e], 'constituency': [constituency],
                                     'age': [a], 'N': [val]})
                census_11 = census_11.append(data)
                age = selection.columns[a + 2]
                data = pd.DataFrame({'sex': [sex], 'ethnicity': [ethnicity], 'constituency': [constituency],
                                     'age': [age], 'N': [val]})
...
census_11.to_csv(script_dir + '/output/census_11_pc16_four_way_joint_distribution.csv', index=False)
...
census_11_str.to_csv(script_dir + '/output/census_11_pc16_four_way_joint_distribution_platform_ready.csv', index=False)

# {'East Midlands': 1, 'Eastern': 2, 'London': 3, 'North East': 4, 'North West': 5, 'South East': 6, 'South West': 7,
#  'Wales': 8, 'West Midlands': 9, 'Yorkshire & the Humber': 10}
